...
Scenario Outline: Prod Server with C drive Only
  Given the server prefix <server_prefix>
  When the servers <start_server_num> through <end_server_num> C drives disk space checked
  Then the C Drive should have at least <c_drive_size> GB
  Examples:
  ...
  Scenario: Prod Util servers
    ...
    When the servers disk space checked include "DS01,AS01,MS01,WEB03,WEB04"
  ...
  Scenario: Prod ADP servers
    ...
    When the servers C drive disk space checked include "BAT11,DFA11,WEB21,WEB22,WEB23,WEB24"
